...
+ function($) {
  ...
  $.showLoading = function(text) {
    ...
    html += '</div>';
    html += '<p class="weui_toast_content">' + (text || "提交订单") + '</p>';
...
//忘记密码
$(document).on("click", "#forget-psw", function() {
        $.actions({
          actions: [{
            text: "手机修改",
            onClick: function() {
              window.location.href="forget-psw-phone.html";
            }
          },{
            text: "邮箱修改",
            onClick: function() {
              window.location.href="forget-psw-email.html";
            }
          }]
        });
      });
...
//加入购车
$(document).on("click", ".add-buy", function() {
        $.toast("预约成功");
      });
//反馈
...
$(document).on("click", "#del-address", function() {
        $.confirm("您确定要删除文此收货地址?", "删除收货地址", function() {
          $.toast("地址已删除!");
        }, function() {
          //取消操作
        });
...
//取消关注
$(document).on("click", "#focus-off", function() {
        $.confirm("您确定要取消关注?", "取消关注", function() {
          $.toast("取消关注成功");
        }, function() {
          //取消操作
        });
...
//首页搜索
	  $(window).scroll(function() {
		if ($(window).scrollTop() > 30.5) {
			$("#search_bar").addClass("fixed")
		} else {
			$("#search_bar").removeClass("fixed")
		}
	});
//
$("#gotop").hide();
...
//属性选择
	$(".choose-type li").click(function(e) {
        $(this).addClass("choose-type-active").siblings().removeClass("choose-type-active")
    });
	$(".more-type i").hide().first().show();
	$(".choose-main").hide();
	$(".more-type").click(function(e) {
		$(".more-type i,.choose-main").toggle();
    });
	//图文详情
	$(".detail-img-size").hide().first().show();
	$(".detail-show-title div").click(function(){
		$(this).addClass("text-danger").siblings().removeClass("text-danger");
		$(".detail-img-size").hide().eq($(this).index()).show();
		$(".up-cart").eq($(this).index()).children().addClass("cart-active");
		$(".up-cart").eq($(this).index()).siblings().children().removeClass("cart-active");

		})
	//关注
	$(".focus-focus i").hide().first().show();
	$(".focus-focus").click(function(e) {
        $(".focus-focus i").toggle();
    });
}($);
